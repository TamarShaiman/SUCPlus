...
class TimeAwareMF:
    ...
    def save_model(self, path):
        ...
        print("Saving U and L...")
        for i in range(self.T):
            np.save(f"{path}U{i}.npy", self.U[i])
        np.save(f"{path}L.npy", self.L)
        print("Done. Elapsed time:", time.time() - ctime, "s")

    def load_model(self, path):
        ctime = time.time()
        print("Loading U and L...")
        self.U = [np.load(f"{path}U{i}.npy") for i in range(self.T)]
        self.L = np.load(f"{path}L.npy")
        self.LT = self.L.T
        print("Done. Elapsed time:", time.time() - ctime, "s")
    ...
    def load_sigma(self, path):
        ctime = time.time()
        print("Loading sigma...")
        sigma = np.load(f"{path}sigma.npy", allow_pickle=True)
        print("Done. Elapsed time:", time.time() - ctime, "s")
        return sigma.tolist()

    def get_t_1(self, t):
        return (t - 1) if t != 0 else (self.T - 1)
    ...
    def get_phi(self, C, i, t):
        ...
        if norm_t == 0 or norm_t_1 == 0:
            return 0.0
        return (C[t][i, :].dot(C[t_1][i, :].T).toarray()[0, 0]) / (norm_t * norm_t_1)
    ...
    def train(self, sparse_check_in_matrices, temp_dir, max_iters=100, load_sigma=False):
        ...
        if load_sigma:
            ...
            sigma = self.init_sigma(C, M, T)
            np.save(f"{temp_dir}/sigma.npy", sigma, allow_pickle=True)
        ...
        for iters in range(max_iters):
            ...
            for t in range(T):
                ...
                numerator = C[t] @ L + Lambda * sigma[t] @ U[t_1]
                denominator = np.maximum(1e-6, C_est[t] @ L + Lambda * sigma[t] @ U[t_1] + alpha * U[t_1])
                U[t] *= np.sqrt(numerator / denominator)

            numerator = np.sum([C[t].T @ U[t] for t in range(T)], axis=0)
            denominator = np.maximum(1e-6, np.sum([C_est[t].T @ U[t] for t in range(T)], axis=0) + beta * L)
            L *= np.sqrt(numerator / denominator)

            error = sum((C_est[t] - C[t]).power(2).sum() for t in range(T))
    ...
    def predict(self, i, j):
        return sum(self.U[t][i] @ self.L[j] for t in range(self.T))
